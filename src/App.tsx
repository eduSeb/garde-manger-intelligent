...
import { Plus, Camera, LogOut, ChefHat, Bell, Settings, ShoppingCart } from 'lucide-react';
...
type ActiveTab = 'stock' | 'recipes' | 'shopping';
  ...
  return (
    <div className="relative min-h-dvh flex items-center justify-center p-6">
      ...
      <motion.div
        ...
        className="relative z-10 glass-panel p-10 w-full max-w-sm text-center"
      >
        <div className="text-6xl mb-2">🥗</div>
        <h1 className="text-3xl font-black text-gradient-green mb-1">Garde-Manger</h1>
        <p className="text-xs uppercase tracking-widest text-[var(--color-text-muted)] mb-8">
          Intelligent v2
        </p>
        <p className="text-sm text-[var(--color-text-secondary)] mb-8 leading-relaxed">
          Gérez votre stock, évitez le gaspillage, cuisinez malin.
        </p>
...
export default function App() {
  ...
  React.useEffect(() => {
    ...
    const unsubRecipes = onSnapshot(qRecipes, (snap) => {
      setUserRecipes(snap.docs.map(d => ({ id: d.id, ...d.data() })) as Recipe[]);
    });

    const unsubShopping = subscribeToShoppingList(user.uid, setShoppingList);

    getDoc(doc(db, 'preferences', user.uid)).then(snap => {
      setPreferences(snap.exists() ? snap.data() as UserPreferences : { ...DEFAULT_PREFERENCES, userId: user.uid });
    });
  ...
  const urgentItems = items.filter(i => {
    if (!i.expiryDate || i.isFrozen) return false;
    const days = Math.ceil((new Date(i.expiryDate).getTime() - Date.now()) / 86400000);
    return days <= (preferences.expiryAlertDays || 3);
  });

  const filteredItems = storageFilter === 'all'
    ? items
    : items.filter(i => i.location === storageFilter);
  ...
  const handleCapture = async (productBase64: string, dlcBase64: string, barcode?: string) => {
    ...
    let barcodeToUse = barcode;

    // Si on a pris une photo du produit, on demande à Gemini de lire l'image
    // pour trouver le nom du produit et éventuellement le code-barres.
    if (productBase64) {
      const { analyzeProductImage } = await import('./lib/gemini');
      const productInfo = await analyzeProductImage(productBase64);
      if (productInfo.name) partial.name = productInfo.name;
      if (productInfo.barcode && !barcodeToUse) barcodeToUse = productInfo.barcode;
    }
    ...
    const expiryDate = await analyzeExpiryDate(dlcBase64);
    if (expiryDate) partial.expiryDate = expiryDate;

    setIsAnalyzing(false);
    setIsScanning(false);
    setEditingItem(partial as FoodItem);
    setShowForm(true);
  };
  ...
  const handleSaveProduct = async (data: Partial<FoodItem>) => {
    ...
    try {
      if (data.id) {
        const { id, ...clean } = data;
        await updateDoc(doc(db, 'items', id), clean);
      } else {
        await addDoc(collection(db, 'items'), { ...data, userId: user.uid, addedAt: new Date().toISOString() });
      }
  ...
  const handleFreeze = async (item: FoodItem) => {
    if (!item.id) return;
    const newExpiry = computeFrozenExpiryDate(preferences.freezerExtensionMonths);
    try {
  ...
  const handleSuggestRecipes = async (filters?: RecipeFilters) => {
    ...
    const priorityItems = urgentItems.map(i => i.name);
    const suggested = await suggestRecipes(items, { ...filters, priorityItems });
  ...
  const handleAddUserRecipe = async (recipe: Recipe) => {
    if (!user) return;
    try { await addDoc(collection(db, 'recipes'), { ...recipe, userId: user.uid }); }
    catch (e) { handleFirestoreError(e, OperationType.WRITE, 'recipes'); }
  };

  // ── Loading
  if (isAuthLoading) return (
    <div className="min-h-dvh flex items-center justify-center bg-[var(--color-brand-bg)]">
      <div className="w-10 h-10 rounded-full border-2 border-[var(--color-accent-green)] border-t-transparent animate-spin" />
    </div>
  );
    ...
    <div className="relative min-h-dvh flex flex-col">
      ...
      <header className="glass-nav sticky top-0 z-50 h-16 flex items-center justify-between px-4 md:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-black text-gradient-green leading-none">Garde-Manger</h1>
            <p className="text-[9px] uppercase tracking-widest text-[var(--color-text-muted)] leading-none mt-0.5">Intelligent v2</p>
          </div>
        </div>
            ...
            >
              {tab === 'stock' ? 'Inventaire' : tab === 'recipes' ? 'Recettes' : (
                <span className="flex items-center gap-1.5">
                  Courses
                  {uncheckedShopping > 0 && <span className="bg-[var(--color-accent-amber)] text-black text-[9px] font-black px-1.5 py-0.5 rounded-full">{uncheckedShopping}</span>}
                </span>
              )}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button className="btn-icon" onClick={() => setShowNotifications(v => !v)}>
              <Bell size={16} />
              {urgentItems.length > 0 && <span className="notification-dot absolute top-0.5 right-0.5" />}
            </button>
            <AnimatePresence>
              {showNotifications && urgentItems.length > 0 && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.95, y: 8 }}
                  animate={{ opacity: 1, scale: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.95, y: 8 }}
                  className="absolute right-0 top-12 glass-panel p-4 w-72 z-50"
                >
                  <p className="label-glass mb-3">⚠️ Bientôt périmés</p>
                  <div className="space-y-2">
                    {urgentItems.map(i => (
                      <div key={i.id} className="glass-card-sm px-3 py-2 flex items-center justify-between">
                        <span className="text-xs font-semibold text-white truncate">{i.name}</span>
                        <span className="badge badge-danger ml-2 shrink-0">{i.expiryDate}</span>
                      </div>
                    ))}
                  </div>
                  <button onClick={() => { handleSuggestRecipes(); setShowNotifications(false); }} className="btn-primary w-full mt-3 text-xs">
          ...
          <button className="btn-primary gap-2 hidden md:inline-flex" onClick={() => setIsScanning(true)}>
      ...
      <main className="relative z-10 flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <aside className="glass-sidebar w-full md:w-72 shrink-0 p-6 flex flex-col gap-6">
          ...
          <div className="glass-card p-5 text-center">
            <p className="text-5xl font-black text-gradient-warm leading-none">
              {String(urgentItems.length).padStart(2, '0')}
            </p>
            <p className="text-[10px] uppercase tracking-widest text-[var(--color-text-muted)] mt-1">Alertes péremption</p>
            {urgentItems.length > 0 && (
              <button onClick={() => handleSuggestRecipes()} className="btn-primary w-full mt-4 text-xs">
                <ChefHat size={13} /> Recettes de sauvetage
              </button>
            )}
          </div>
          ...
          <div className="glass-card p-5 space-y-3">
            ...
            {[
              { loc: StorageLocation.FRIDGE, label: 'Frigo', emoji: '❄️', color: 'var(--color-accent-blue)' },
              { loc: StorageLocation.PANTRY, label: 'Placard', emoji: '🥫', color: 'var(--color-accent-amber)' },
              { loc: StorageLocation.FREEZER, label: 'Congélo', emoji: '🧊', color: 'var(--color-accent-purple)' },
            ].map(({ loc, label, emoji, color }) => {
          ...
          {/* Ajouter un produit (mobile) */}
          <div className="md:hidden grid grid-cols-2 gap-2">
            <button className="btn-primary" onClick={() => setIsScanning(true)}>
              <Camera size={14} /> Scanner
            </button>
            <button className="btn-ghost" onClick={() => { setEditingItem(null); setShowForm(true); }}>
              <Plus size={14} /> Manuel
            </button>
          </div>
        </aside>
        ...
        <section className="flex-1 p-6 overflow-y-auto custom-scroll">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div className="storage-tabs">
                  {STORAGE_TABS.map(t => (
                    <button
                      key={t.key}
                      onClick={() => setStorageFilter(t.key)}
                      className={\`storage-tab \${storageFilter === t.key ? 'active ' + (t.cls || '') : ''}\`}
                    >
                      {t.emoji} {t.label}
                      <span className="opacity-60 text-[10px]">
                        ({t.key === 'all' ? items.length : items.filter(i => i.location === t.key).length})
                      </span>
                    </button>
                  ))}
                </div>
                <button className="btn-ghost hidden md:inline-flex" onClick={() => { setEditingItem(null); setShowForm(true); }}>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              {isRecipeLoading ? (
                ...
                <RecipeList
                  ...
                  onFilter={handleSuggestRecipes}
                  onAddManual={handleAddUserRecipe}
                />
              )}
            </div>
          )}
          ...
          {activeTab === 'shopping' && (
            <ShoppingListView
              ...
              onClearChecked={() => clearCheckedItems(shoppingList)}
            />
          )}
        </section>
      </main>
      ...
      <nav className="glass-nav md:hidden fixed bottom-0 inset-x-0 z-50 flex">
        {([
          { key: 'stock', emoji: '📦', label: 'Stock' },
          { key: 'recipes', emoji: '🍳', label: 'Recettes' },
          { key: 'shopping', emoji: '🛒', label: \`Courses\${uncheckedShopping > 0 ? \` (\${uncheckedShopping})\` : ''}\` },
        ] as const).map(t => (
          <button
            key={t.key}
            onClick={() => t.key === 'recipes' ? handleSuggestRecipes() : setActiveTab(t.key)}
      ...
      <AnimatePresence>
        {isScanning && (
          <CameraModal onClose={() => setIsScanning(false)} onCapture={handleCapture} isLoading={isAnalyzing} />
        )}
        {showForm && (
          <ProductForm initialData={editingItem || undefined} onClose={() => { setShowForm(false); setEditingItem(null); }} onSave={handleSaveProduct} />
        )}
        {showSettings && (
          <SettingsModal preferences={preferences} onClose={() => setShowSettings(false)} onSave={handleSavePreferences} />
        )}
        {consumeTarget && (
          <ConsumeModal item={consumeTarget} onCancel={() => setConsumeTarget(null)} onConfirm={confirmConsume} />
        )}
      </AnimatePresence>
    </div>
  );
...
function ShoppingListView({ items, onToggle, onRemove, onClearChecked }: {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        {checked.length > 0 && (
          <button onClick={onClearChecked} className="btn-ghost text-xs">
            Effacer les cochés ({checked.length})
          </button>
        )}
      </div>
      ...
      {items.length === 0 ? (
        <div className="glass-card p-12 text-center">
          ...
          <p className="text-[var(--color-text-muted)] text-sm">Votre liste est vide.<br/>Elle se remplira quand vous consommerez des produits.</p>
        </div>
      ) : (
        <>
          {unchecked.length > 0 && (
          ...
          {checked.length > 0 && (
            <div className="space-y-2 opacity-50">
              <p className="label-glass">Achetés</p>
              {checked.map(item => (
                <div key={item.id} className="glass-card-sm flex items-center gap-3 px-4 py-3">
...
function ConsumeModal({ item, onCancel, onConfirm }: {
  item: FoodItem;
  onCancel: () => void;
  onConfirm: (addToList: boolean) => void;
}) {
  return (
    <motion.div className="glass-overlay fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    >
      <motion.div className="glass-panel w-full max-w-sm p-6"
        initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }}
      >
        <p className="text-2xl mb-1">✅</p>
        <h3 className="text-lg font-bold text-white mb-1">Produit consommé</h3>
        <p className="text-sm text-[var(--color-text-secondary)] mb-6">
          Voulez-vous ajouter <strong className="text-white">"{item.name}"</strong> à votre liste de courses ?
        </p>
        <div className="flex flex-col gap-2">
          <button className="btn-primary" onClick={() => onConfirm(true)}>
            🛒 Oui, ajouter à la liste
          </button>
          <button className="btn-ghost" onClick={() => onConfirm(false)}>
            Non merci
          </button>
          <button className="btn-ghost" onClick={onCancel} style={{ color: 'var(--color-text-muted)' }}>
            Annuler
          </button>
        </div>
      </motion.div>
